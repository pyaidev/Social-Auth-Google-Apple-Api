...
class FavouriteZikrListViewTestCase(APITestCase):
    def setUp(self):
    ...
    def test_favourite_zikr_list(self):
        ...
        url = reverse("users:FavouriteZikrList")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(response.data["results"][0]["id"], self.zikr.id)
        self.assertEqual(response.data["results"][0]["title"], self.zikr.title)
